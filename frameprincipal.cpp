...
{
    df.gerarObjetos();
    transladarWindowParaOrigem();
}

void FramePrincipal::transladarWindowParaOrigem() {
    float Wcx = (xMin + xMax) / 2.0;
    float Wcy = (yMin + yMax) / 2.0;

    Matriz matrizTransladaWcParaOrigem = Matriz::translacao(-Wcx, -Wcy);

    for (Objeto &obj : df.displayFile) {
        obj.transformarPontos(matrizTransladaWcParaOrigem);
    }
}
...
void FramePrincipal::transformarObjeto(const QString &inputText) {
    if (objAtual) {
        Matriz transformacao = Matriz::translacao(10, 10);  // Exemplo de transformação
        objAtual->transformarPontos(transformacao);
        update();
    }
}
